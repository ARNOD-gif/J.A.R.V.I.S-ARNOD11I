...
# Guarded for cross-platform compatibility (Android/Linux safe)
...
# ── Kivy UI Fallback for Android Compatibility ──────────────────────────────
class KivyJarvisUI:
    """Android/Kivy cross-platform wrapper for Jarvis UI."""
    def __init__(self, face_image="face.png"):
        ...
        self.on_weather_requested = None
        self._win = type("Win", (), {"_ready": True})()
    ...
    def show_content(self, label, content):
        print(f"[UI Content] {label}:\n{content}")

    def update_weather(self, weather_data):
        print(f"[UI Weather]: {weather_data}")

    def start_camera_stream(self):
        print("[UI Camera]: Camera stream started")

    def stop_camera_stream(self):
        print("[UI Camera]: Camera stream stopped")

    def notify_phone_connected(self):
        print("[UI Phone]: Phone connected")

    def get_plugins(self):
        return []

    def request_say(self, instruction):
        pass
    ...
    def run_app(self):
        ...
        class JarvisApp(App):
            def build(inner_self):
                ...
                inner_self.label = Label(text="J.A.R.V.I.S Active & Listening...", font_size='20sp')
                layout.add_widget(inner_self.label)
                return layout

        JarvisApp().run()

JarvisUI = KivyJarvisUI
...
def _weather_icon(code: int) -> tuple[str, str]:
    """Map Open-Meteo weather code to a compact HUD icon and label."""
    mapping = {
        ...
        95: ("ϟ", "Thunderstorm"), 96: ("ϟ", "Storm + Hail"), 99: ("ϟ", "Storm + Hail"),
    }
    return mapping.get(code, ("◌", "Unknown"))

def fetch_weather_for_ui(city: str) -> dict:
    ...
    req = Request(geo_url, headers={"User-Agent": "JARVIS-HUD/1.0"})
    ...
    forecast_url = (
        ...
        f"latitude={lat}&longitude={lon}&current=temperature_2m,relative_humidity_2m,precipitation,weather_code,wind_speed_10m"
        "&hourly=precipitation_probability&forecast_days=1&timezone=auto"
    )
    req = Request(forecast_url, headers={"User-Agent": "JARVIS-HUD/1.0"})
    with urlopen(req, timeout=10) as r:
        data = json.loads(r.read().decode("utf-8"))
    cur = data.get("current", {})
    code = int(cur.get("weather_code", -1))
    icon, condition = _weather_icon(code)
    probs = data.get("hourly", {}).get("precipitation_probability", [])
    rain = int(max(probs[:1] or [0]))
    return {
...
def main():
    ...
    def runner():
        def weather_request(city: str):
            def worker():
                try:
                    ui.update_weather(fetch_weather_for_ui(city))
                    ui.write_log(f"WX: Weather updated — {city}")
                except Exception as e:
                    ui.update_weather({"error": f"{type(e).__name__}: {e}"})
                    ui.write_log(f"ERR: Weather — {type(e).__name__}: {e}")
            threading.Thread(target=worker, daemon=True, name="WeatherWorker").start()

        ui.on_weather_requested = weather_request
        weather_request("Rairangpur, Odisha")
