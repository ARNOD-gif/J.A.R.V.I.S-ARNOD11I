...
class HudCanvas(QWidget):
    ...
    def _draw_high_graphics_background(self, p, W, H, now):
        """Premium sci-fi backdrop: vignette, scanlines, HUD ticks and moving data grid."""
        # Soft radial glow behind the reactor.
        cx, cy = W / 2, H / 2
        glow = QRadialGradient(QPointF(cx, cy), max(W, H) * 0.58)
        glow.setColorAt(0.00, qcol(C.PRI_GHO, 105))
        glow.setColorAt(0.28, qcol(C.PRI_GHO, 48))
        glow.setColorAt(0.62, qcol(C.BG, 220))
        glow.setColorAt(1.00, qcol(C.BG, 255))
        p.fillRect(self.rect(), QBrush(glow))

        # Fine scanlines give the display a high-end monitor/HUD texture.
        p.setPen(QPen(qcol(C.PRI, 11), 1))
        for y in range(0, H, 4):
            p.drawLine(QPointF(0, y), QPointF(W, y))

        # Perspective-ish micro grid, denser near the edges.
        p.setPen(QPen(qcol(C.PRI_DIM, 30), 1))
        grid = max(24, int(min(W, H) * 0.045))
        offset = int((now * 8) % grid)
        for x in range(-grid + offset, W + grid, grid):
            p.drawLine(QPointF(x, 0), QPointF(x + (x - cx) * 0.035, H))
        for y in range(-grid + offset, H + grid, grid):
            p.drawLine(QPointF(0, y), QPointF(W, y + (y - cy) * 0.02))

        # Edge telemetry ticks.
        p.setPen(QPen(qcol(C.BORDER_B, 105), 1))
        for x in range(36, W - 35, 36):
            ln = 6 if x % 144 else 11
            p.drawLine(QPointF(x, 7), QPointF(x, 7 + ln))
            p.drawLine(QPointF(x, H - 7), QPointF(x, H - 7 - ln))
        for y in range(36, H - 35, 36):
            ln = 6 if y % 144 else 11
            p.drawLine(QPointF(7, y), QPointF(7 + ln, y))
            p.drawLine(QPointF(W - 7, y), QPointF(W - 7 - ln, y))

        # Moving scan beam across the full viewport.
        scan_y = (now * (28 if self.speaking else 10)) % (H + 180) - 90
        beam = QLinearGradient(0, scan_y - 70, 0, scan_y + 70)
        beam.setColorAt(0.0, qcol(C.PRI, 0))
        beam.setColorAt(0.5, qcol(C.PRI, 20 if not self.speaking else 34))
        beam.setColorAt(1.0, qcol(C.PRI, 0))
        p.fillRect(QRectF(0, scan_y - 70, W, 140), QBrush(beam))

    def _draw_hud_nodes(self, p, W, H, now):
        """Decorative tracking nodes, brackets and telemetry labels around the core."""
        cx, cy = W / 2, H / 2
        fw = min(W, H)
        r = fw * 0.315
        nodes = []
        for i in range(8):
            a = now * (0.22 if not self.speaking else 0.42) + i * math.pi / 4
            wob = math.sin(now * 1.4 + i) * fw * 0.006
            nodes.append((cx + math.cos(a) * (r + wob), cy + math.sin(a) * (r + wob)))

        for i, (nx, ny) in enumerate(nodes):
            p.setPen(QPen(qcol(C.PRI if i % 3 else C.ACC, 135), 1))
            p.setBrush(QBrush(qcol(C.BG, 220)))
            p.drawEllipse(QPointF(nx, ny), 4.5, 4.5)
            p.drawEllipse(QPointF(nx, ny), 10, 10)
            # tiny connector to the reactor perimeter
            a = math.atan2(ny - cy, nx - cx)
            x2 = cx + math.cos(a) * r * 0.82
            y2 = cy + math.sin(a) * r * 0.82
            p.drawLine(QPointF(x2, y2), QPointF(nx, ny))

        # Four large targeting brackets.
        br = fw * 0.285
        gap = fw * 0.035
        ...
        for a in (0, math.pi/2, math.pi, 3*math.pi/2):
            ...
            vx, vy = -uy, ux
            x = cx + ux * br; y = cy + uy * br
            p.drawLine(QPointF(x - vx*gap, y - vy*gap), QPointF(x + ux*gap*1.8 - vx*gap, y + uy*gap*1.8 - vy*gap))
            p.drawLine(QPointF(x + vx*gap, y + vy*gap), QPointF(x + ux*gap*1.8 + vx*gap, y + uy*gap*1.8 + vy*gap))

        p.setFont(QFont("Courier New", 6, QFont.Weight.Bold))
        labels = [("TRACK 01", -0.82, -0.34), ("VECTOR 7A", 0.50, -0.38),
                  ("AUTH // OK", -0.82, 0.43), ("LINK 100%", 0.52, 0.43)]
        for txt, ax, ay in labels:
            p.setPen(QPen(qcol(C.TEXT_DIM, 170), 1))
            p.drawText(QRectF(cx + ax*fw/2, cy + ay*fw/2, 100, 14), Qt.AlignmentFlag.AlignLeft, txt)

    def _draw_core_glow(self, p, cx, cy, r0, now):
        """Layered additive-looking glow around the reactor core."""
        radius = r0 * (1.55 + 0.08 * math.sin(now * 2.2))
        grad = QRadialGradient(QPointF(cx, cy), radius)
        grad.setColorAt(0.00, qcol(C.WHITE, 100 if self.speaking else 65))
        grad.setColorAt(0.10, qcol(C.PRI, 105 if self.speaking else 75))
        grad.setColorAt(0.42, qcol(C.PRI, 34))
        grad.setColorAt(0.78, qcol(C.PRI, 8))
        grad.setColorAt(1.00, qcol(C.PRI, 0))
        ...
        p.setPen(QPen(qcol(C.WHITE, 115 if self.speaking else 75), 1.1))
        blade_r = r0 * 0.92
        for i in range(3):
            a = now * (1.3 if self.speaking else 0.55) + i * 2.094
            p.drawArc(QRectF(cx-blade_r, cy-blade_r, blade_r*2, blade_r*2), int((math.degrees(a) % 360.0) * 16), 24*16)
    ...
    def paintEvent(self, _):
        ...
        self._draw_high_graphics_background(p, W, H, now)
        p.setPen(QPen(qcol(C.PRI_GHO, 105), 1))
        grid = 42
        ...
        for x in range(0, W, 168):
            p.drawLine(QPointF(x, 0), QPointF(x, H))
        for y in range(0, H, 126):
            p.drawLine(QPointF(0, y), QPointF(W, y))

        # HUD frame / corner brackets.
        margin = max(12, int(fw * 0.025))
        ...
        # ── Animated central reactor / radar ───────────────────────────────
        r0 = fw * 0.185
        self._draw_core_glow(p, cx, cy, r0, now)
        halo = self._halo
        for i in range(12, 0, -1):
            r = r0 * (1.0 + i * 0.105)
            a = max(0, min(255, int(halo * 0.035 * (13-i))))
            p.setPen(QPen(qcol(C.MUTED_C if self.muted else C.PRI, a), 1))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawEllipse(QRectF(cx-r, cy-r, 2*r, 2*r))

        # Multiple animated segmented rings.
        rings = [
            (r0*1.00, 3.0, 92, 54, self._rings[0]),
            (r0*1.24, 2.0, 70, 42, self._rings[1]),
            (r0*1.52, 1.5, 52, 30, self._rings[2]),
        ]
        ring_col = C.MUTED_C if self.muted else C.PRI
        for rr, width, arc_len, gap, base in rings:
            p.setPen(QPen(qcol(ring_col, min(245, int(75 + halo))), width))
        ...
        # Radar sweep beams.
        sweep_r = r0 * 1.72
        srect = QRectF(cx-sweep_r, cy-sweep_r, sweep_r*2, sweep_r*2)
        p.setPen(QPen(qcol(ring_col, min(230, int(100 + halo))), 2.5))
        p.drawArc(srect, int(self._scan*16), int((90 if self.speaking else 55)*16))
        p.setPen(QPen(qcol(C.ACC, min(170, int(55 + halo/2))), 1.2))
        p.drawArc(srect, int(self._scan2*16), int(38*16))

        # Crosshair + central core.
        cross = r0 * 1.95
        gap = r0 * 0.46
        p.setPen(QPen(qcol(C.PRI, int(80 + min(130, halo))), 1))
        p.drawLine(QPointF(cx-cross, cy), QPointF(cx-gap, cy))
        p.drawLine(QPointF(cx+gap, cy), QPointF(cx+cross, cy))
        p.drawLine(QPointF(cx, cy-cross), QPointF(cx, cy-gap))
        p.drawLine(QPointF(cx, cy+gap), QPointF(cx, cy+cross))

        core_r = r0 * 0.52 * self._scale
        for i in range(9, 0, -1):
            rr = core_r * i / 9
            a = int((28 + halo*0.45) * (i/9))
            p.setBrush(QBrush(qcol(C.MUTED_C if self.muted else C.PRI, max(5, min(150, a)))))
            p.setPen(Qt.PenStyle.NoPen)
            p.drawEllipse(QRectF(cx-rr, cy-rr, rr*2, rr*2))
        p.setPen(QPen(qcol(C.WHITE, 210), 1))
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawEllipse(QRectF(cx-core_r, cy-core_r, core_r*2, core_r*2))
        p.drawEllipse(QRectF(cx-core_r*0.55, cy-core_r*0.55, core_r*1.1, core_r*1.1))

        # ── Left data panel: system metrics ─────────────────────────────────
        def panel(x, y, w, h, title):
            p.setBrush(QBrush(qcol(C.PANEL, 215)))
            p.setPen(QPen(qcol(C.BORDER_B, 190), 1))
            p.drawRoundedRect(QRectF(x, y, w, h), 4, 4)
            p.setPen(QPen(qcol(C.PRI, 180), 1))
            p.drawLine(QPointF(x+10, y+24), QPointF(x+w-10, y+24))
            p.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
            p.setPen(QPen(qcol(C.PRI, 220), 1))
            p.drawText(QRectF(x+10, y+7, w-20, 14), Qt.AlignmentFlag.AlignLeft, title)

        px = margin + 12
        pw = max(150, int(W * 0.18))
        py = margin + 22
        ph = min(168, max(142, int(H*0.28)))
        panel(px, py, pw, ph, "SYSTEM TELEMETRY")
        metrics = [
            ("CPU", snap.get("cpu", 0), C.PRI),
            ("MEM", snap.get("mem", 0), C.ACC2),
            ("GPU", max(0, snap.get("gpu", 0)), C.ACC),
            ("NET", min(100, snap.get("net", 0)*10), C.GREEN),
        ]
        by = py + 38
        for name, val, col in metrics:
            val = float(val if val >= 0 else 0)
            p.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
            p.setPen(QPen(qcol(C.TEXT_MED), 1))
            p.drawText(QRectF(px+10, by, 35, 13), Qt.AlignmentFlag.AlignLeft, name)
            p.setPen(QPen(qcol(col, 190), 1))
            p.drawRect(QRectF(px+48, by+3, pw-62, 6))
            p.setBrush(QBrush(qcol(C.BAR_BG, 220))); p.drawRect(QRectF(px+48, by+3, pw-62, 6))
            p.setBrush(QBrush(qcol(col, 215))); p.drawRect(QRectF(px+48, by+3, (pw-62)*min(100,val)/100, 6))
            p.setPen(QPen(qcol(col, 230), 1))
            p.drawText(QRectF(px+48, by+12, pw-62, 12), Qt.AlignmentFlag.AlignRight, f"{val:05.1f}%")
            by += 29

        # ── Top-left globe-like status widget ────────────────────────────────
        gr = min(38, fw*0.075)
        gx, gy = px + gr + 4, py + ph + gr + 14
        p.setBrush(QBrush(qcol(C.PANEL2, 180)))
        p.setPen(QPen(qcol(C.BORDER_B, 170), 1))
        p.drawEllipse(QRectF(gx-gr, gy-gr, gr*2, gr*2))
        p.setPen(QPen(qcol(C.PRI, 155), 1))
        p.drawEllipse(QRectF(gx-gr*0.58, gy-gr, gr*1.16, gr*2))
        p.drawEllipse(QRectF(gx-gr, gy-gr*0.45, gr*2, gr*0.9))
        p.drawArc(QRectF(gx-gr*1.16, gy-gr*1.16, gr*2.32, gr*2.32), int(self._scan*8), 90*16)
        p.setFont(QFont("Courier New", 6, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.TEXT_DIM), 1))
        p.drawText(QRectF(gx+gr+8, gy-20, 110, 12), Qt.AlignmentFlag.AlignLeft, "NODE 01  ·  ONLINE")
        p.drawText(QRectF(gx+gr+8, gy-5, 110, 12), Qt.AlignmentFlag.AlignLeft, "LINK  /  STABLE")
        p.drawText(QRectF(gx+gr+8, gy+10, 110, 12), Qt.AlignmentFlag.AlignLeft, "SEC  /  CLEARED")

        # ── Top-right wireframe visualization ────────────────────────────────
        wx = W - margin - max(185, int(W*0.23))
        wy = margin + 18
        ww = max(185, int(W*0.23)); wh = min(142, max(118, int(H*0.23)))
        panel(wx, wy, ww, wh, "TACTICAL MATRIX")
        poly = [
            QPointF(wx+ww*0.25, wy+wh*0.48),
            QPointF(wx+ww*0.73, wy+wh*0.35),
            QPointF(wx+ww*0.58, wy+wh*0.72),
            QPointF(wx+ww*0.37, wy+wh*0.80),
        ]
        p.setPen(QPen(qcol(C.PRI, 155), 1))
        p.setBrush(Qt.BrushStyle.NoBrush)
        ...
        for i in range(1, 6):
            a = i/6
            p.drawLine(QPointF(wx+ww*(0.25+a*0.48), wy+wh*0.48), QPointF(wx+ww*(0.25+a*0.33), wy+wh*0.80))
        for i in range(1, 5):
            a=i/5
            p.drawLine(QPointF(wx+ww*(0.25+a*0.48), wy+wh*(0.48+a*0.24)), QPointF(wx+ww*(0.25+a*0.33), wy+wh*(0.80-a*0.08)))
        p.setPen(QPen(qcol(C.ACC, 180), 1.5))
        node_ang = now * 1.4
        nx = wx+ww*0.49 + math.cos(node_ang)*ww*0.16
        ny = wy+wh*0.61 + math.sin(node_ang)*wh*0.14
        p.drawEllipse(QRectF(nx-3, ny-3, 6, 6))

        # ── Bottom-left animated bar graph ───────────────────────────────────
        bx, by = px, H - margin - 128
        bw, bh = pw, 108
        panel(bx, by, bw, bh, "ACTIVITY VECTOR")
        p.setPen(QPen(qcol(C.BORDER, 150), 1))
        p.drawLine(QPointF(bx+10, by+bh-18), QPointF(bx+bw-10, by+bh-18))
        bars = [0.35, 0.62, 0.47, 0.82, 0.58, 0.91, 0.44, 0.70, 0.53, 0.77]
        for i, base in enumerate(bars):
            wobble = 0.18 * math.sin(self._tick*0.10 + i*0.75) if self.speaking else 0.06*math.sin(self._tick*0.05+i)
            bhh = max(4, (base+wobble) * (bh-34))
            xx = bx+12+i*((bw-24)/len(bars))
            p.fillRect(QRectF(xx, by+bh-18-bhh, max(3,(bw-30)/len(bars)), bhh), qcol(C.PRI if i%3 else C.ACC, 175))
        p.setFont(QFont("Courier New", 6))
        p.setPen(QPen(qcol(C.TEXT_DIM), 1))
        p.drawText(QRectF(bx+10, by+bh-15, bw-20, 12), Qt.AlignmentFlag.AlignLeft, "00  15  30  45  60  75  90  105")

        # ── Bottom-right signal / diagnostic panel ───────────────────────────
        sx = W - margin - ww
        sy = H - margin - 128
        sh = 108
        panel(sx, sy, ww, sh, "SIGNAL ANALYSIS")
        p.setPen(QPen(qcol(C.BORDER, 140), 1))
        midy = sy + sh*0.62
        p.drawLine(QPointF(sx+10, midy), QPointF(sx+ww-10, midy))
        pts=[]
        for i in range(34):
            xx = sx+10+i*(ww-20)/33
            amp = (10 if self.speaking else 5) * math.sin(self._tick*0.20+i*0.72)
            amp += 4*math.sin(self._tick*0.08+i*0.31)
            pts.append(QPointF(xx, midy-amp))
        if pts:
            path=QPainterPath(pts[0])
            for pt in pts[1:]: path.lineTo(pt)
            p.setPen(QPen(qcol(C.PRI, 205), 1.4)); p.drawPath(path)
        p.setFont(QFont("Courier New", 6, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.TEXT_MED), 1))
        p.drawText(QRectF(sx+10, sy+32, ww-20, 13), Qt.AlignmentFlag.AlignLeft, "CORE  STABLE")
        p.drawText(QRectF(sx+10, sy+47, ww-20, 13), Qt.AlignmentFlag.AlignLeft, f"TEMP  {snap.get('tmp', -1):.0f} C" if snap.get('tmp', -1) >= 0 else "TEMP  N/A")
        p.drawText(QRectF(sx+10, sy+62, ww-20, 13), Qt.AlignmentFlag.AlignLeft, "ENCRYPTION  AES-LINK")
        p.setPen(QPen(qcol(C.GREEN, 180), 1))
        p.drawText(QRectF(sx+ww-82, sy+32, 70, 13), Qt.AlignmentFlag.AlignRight, "SYNC  100%")

        # ── State + animated waveform ────────────────────────────────────────
        sy0 = cy + r0*2.05
        if self.muted:
            ...
            txt, col = ("◈  THINKING" if self._blink else "◇  THINKING"), qcol(C.ACC2)
        elif self.state == "PROCESSING":
            txt, col = ("▷  PROCESSING" if self._blink else "▶  PROCESSING"), qcol(C.ACC2)
        elif self.state == "LISTENING":
            txt, col = ("●  LISTENING" if self._blink else "○  LISTENING"), qcol(C.GREEN)
        else:
            txt, col = ("●  " + self.state), qcol(C.PRI)
        p.setPen(QPen(col, 1))
        p.setFont(QFont("Courier New", 10, QFont.Weight.Bold))
        p.drawText(QRectF(cx-160, sy0, 320, 24), Qt.AlignmentFlag.AlignCenter, txt)
        ...
        for i in range(N):
            if self.muted:
                ...
                hgt = 4 + 22*(0.5+0.5*math.sin(self._tick*0.35+i*0.55))
                cl = qcol(C.PRI if i%4 else C.ACC, 210)
            else:
                hgt = 3 + 2*math.sin(self._tick*0.09+i*0.6)
                cl = qcol(C.BORDER_B, 180)
            p.fillRect(QRectF(wx0+i*bwave, wy+22-hgt, bwave-1, hgt), cl)

        # Premium targeting/data layer around the reactor.
        self._draw_hud_nodes(p, W, H, now)
        ...
        # Tiny coordinate readout + 12h clock inside the HUD.
        p.setFont(QFont("Courier New", 6, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.TEXT_DIM, 200), 1))
        p.drawText(QRectF(W-margin-190, H-margin-22, 190, 12), Qt.AlignmentFlag.AlignRight,
                   time.strftime("LOCAL  %I:%M:%S %p"))
        p.drawText(QRectF(margin+12, H-margin-22, 190, 12), Qt.AlignmentFlag.AlignLeft,
                   f"HUD  {W:04d}x{H:04d}  ·  T+{self._tick:06d}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle(f"{_display} — MARK XLIII ")
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("MARK 43", C.PRI_DIM))
        lay.addSpacing(8)
        ...
        _sub_text = (" Just A Rather Vibrant Intelligent System"
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        lay.addWidget(_sec("LIVE ACTIVITY // SYSTEM LOG"))
        self._log = LogWidget()
        ...
        lay.addWidget(_sec("DATA INGEST // FILE UPLOAD"))
        ...
        lay.addWidget(_sec("COMMAND LINK // INPUT"))
    ...
    def _show_content(self, title: str, text: str):
        ...
        self._content_ts_lbl.setText(_time.strftime("%I:%M:%S %p"))
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By Arnod", C.PRI_DIM))
        return w
